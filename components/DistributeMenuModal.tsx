      ...
      <div className="w-full max-w-sm rounded-2xl shadow-xl max-h-[92vh] overflow-y-auto"
        ...
        <div className="p-5 space-y-4">
          ...
          {!preselectedMenuId && (
            <div>
              ...
              {menus.length === 0 ? (
                <p className="text-xs" style={{ color: '#94a3b8' }}>Noch keine Menüs. Erstelle zuerst ein Großes Menü.</p>
              ) : (
            ...
            <div className="px-3 py-2.5 rounded-xl flex items-center justify-between"
              ...
              <div>
                ...
                <p className="text-xs mt-0.5" style={{ color: '#16a34a' }}>
                  Geplant für {selectedMeta.num_days} Tag{selectedMeta.num_days > 1 ? 'e' : ''} · Tage anklicken zum Auswählen
                </p>
              </div>
              {selectedDates.size > 0 && (
                <div className="text-right shrink-0 ml-3">
                  <p className="text-lg font-bold leading-tight" style={{ color: '#059669' }}>{selectedDates.size}</p>
                  <p className="text-[10px]" style={{ color: '#16a34a' }}>Tage gewählt</p>
                </div>
              )}
            </div>
          )}
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-medium" style={{ color: '#64748b' }}>Tage auswählen (Mehrfach möglich)</p>
              {selectedDates.size > 0 && (
                <button onClick={() => setSelectedDates(new Set())} className="text-xs" style={{ color: '#94a3b8' }}>Auswahl löschen</button>
              )}
            </div>
          ...
          {sortedSelected.length > 0 && (
            <div>
              <p className="text-xs font-medium mb-1.5" style={{ color: '#64748b' }}>
                {sortedSelected.length} Tag{sortedSelected.length > 1 ? 'e' : ''} ausgewählt
                {selectedMeta && sortedSelected.length !== selectedMeta.num_days && (
                  <span style={{ color: '#f59e0b' }}> · Menü war für {selectedMeta.num_days} Tage geplant</span>
                )}
              </p>
                ...
                {sortedSelected.slice(0, 5).map((d, i) => {
                  ...
                  return (
                    <div key={d} className="flex items-center justify-between px-2 py-1.5 rounded-lg"
                      ...
                      <button onClick={() => toggleDate(d)} className="text-xs" style={{ color: '#94a3b8' }}>×</button>
                    </div>
                  )
                })}
                {sortedSelected.length > 5 && (
                  <p className="text-xs px-2" style={{ color: '#94a3b8' }}>+{sortedSelected.length - 5} weitere…</p>
                )}
              </div>
            </div>
          )}
        ...
        <div className="px-5 pb-5 flex gap-2">
          ...
          <button onClick={distribute}
            ...
            {distributing ? 'Verteilen…' : selectedDates.size > 0 ? `${selectedDates.size}× Verteilen` : 'Verteilen'}
          </button>
        </div>
      </div>
    </div>
