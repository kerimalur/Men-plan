  ...
  async function copy() {
    ...
    if (existing) {
      ...
      const { data: created } = await supabase
        .from('meal_plans')
        .insert({ date: targetDate, kcal_total: 0, protein_total: 0, cost_total: 0 })
        .select()
        .single()
      ...
      >
        ...
        <div className="p-5 space-y-4">
          <p className="text-xs" style={{ color: '#64748b' }}>
            <span className="font-semibold" style={{ color: '#1e293b' }}>{meal.name}</span>{' '}
            kopieren nach:
          </p>
          <input
            type="date"
